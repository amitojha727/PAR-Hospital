			...
			<div class="col-md-12">
				<div style="box-shadow:0px 0px 10px 0px rgb(200,200,200); padding:10px 10px; position:relative; padding-top:20px;">
					<div style="width:40%; position:absolute; top:10px; right:10px; z-index:999;" align="right">
						...
						<a href="{{ route('admin.siteAdd') }}" class="btn btn-info" style="padding:6px 10px; margin-top:-3px;">New Department</a>
					</div>
					<table id="example" class="display nowrap" style="width:100%">
						...
						<tbody id="tb2">
							...
							<tr style="border-bottom:1px solid #F7F7F7;">
							 <td style="padding:0px 10px;">{{ $row->site_id }}</td>
							 <td style="padding:0px 10px;">{{ $row->site_nm }}</td>
							 <td style="padding:10px 10px;">
								<a href="{{ route('admin.siteAdd',['site_id' => $row->site_id]) }}" class="btn btn-warning btn-sm">Edit</a>
								<a href="{{ route('admin.siteDelete',['site_id' => $row->site_id]) }}" class="btn btn-danger btn-sm" onClick="return delSchool()">Delete</a>
							 </td>
							</tr>
							@endforeach
						</tbody>
...
<script>
	function delSchool(){
		var r = confirm("Are you want to delete this Department?");
	  	if (r == true) {
			return true;
	  	} else {
			return false;
