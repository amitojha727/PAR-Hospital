					...
					<table id="example" class="display nowrap" style="width:100%">
						...
						<tbody id="tb2">
							...
							<tr style="border-bottom:1px solid #F7F7F7;">
                                <td style="padding:0px 10px;">{{ $row->appoinment_id }}</td>
                                <td style="padding:0px 10px;">{{ DB::table('site_mast')->where('site_id',$row->site_id)->value('site_nm') }}</td>
                                <td style="padding:0px 10px;">{{ DB::table('employee_mast')->where('emp_id',$row->emp_id)->value('emp_fname') }} {{ DB::table('employee_mast')->where('emp_id',$row->emp_id)->value('emp_sname') }}</td>
                                <td style="padding:0px 10px;">{{ $row->applicant_name }}</td>
                                <td style="padding:0px 10px;">{{ $row->appoinment_date }}</td>
                                <td style="padding:10px 10px;">{{ $row->appoinment_time }}</td>
                                <td style="padding:10px 10px;">{{ $row->applicant_contact_number }}</td>
                                <td style="padding:10px 10px;">{{ $row->message }}</td>
							</tr>
							@endforeach
						</tbody>
					</table>
